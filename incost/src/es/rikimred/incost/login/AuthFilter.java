...
public class AuthFilter implements Filter {
	...
	public void doFilter(final ServletRequest request, final ServletResponse response,
		...
		try {
			...
			// allow user to proccede if url is login.xhtml or user logged in or user is accessing
			// any page in //public folder
			final String reqURI = req.getRequestURI();
			if (reqURI.indexOf("/login.xhtml") >= 0 || ses != null
					&& ses.getAttribute(SessionAttributesEnum.USERNAME.getName()) != null
					|| reqURI.indexOf("/public/") >= 0 || reqURI.contains("javax.faces.resource")) {
				chain.doFilter(request, response);
			}
			else {
				// Usuario anonimo. Redirigir a la pagina de login
